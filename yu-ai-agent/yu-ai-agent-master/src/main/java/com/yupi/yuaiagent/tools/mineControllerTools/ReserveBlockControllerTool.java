...
public class ReserveBlockControllerTool {
    ...
    @Tool(description = "资源储量块段信息：通过块段编号,矿区编码,煤层编号,块段类型,地质储量,可采储量,设计回采率增加新的资源储量块段信息，返回：成功数量，-1表示失败！")
    public int add(
    ...
    @Tool(description = "资源储量块段信息：通过块段编号查询资源储量块段信息，返回：资源储量块段信息")
    ...
    @Tool(description = "资源储量块段信息：查询所有资源储量块段信息，返回：所有资源储量块段信息")
    ...
    @Tool(description = "资源储量块段信息：通过矿区编码查询资源储量块段信息，返回：资源储量块段信息列表")
